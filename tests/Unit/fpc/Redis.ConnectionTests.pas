...
{ Testes de Redis.Connection (FPCUnit). Mesma cobertura do tests\Unit\Redis.ConnectionTests.pas
...
uses
  ...
  Redis.Types, Redis.Resp, Redis.Connection, Redis.Transport;
...
type
  ...
  TRedisPipelineTests = class(TTestCase)
  published
    procedure Vazio_NaoTemComando;
    procedure Queue_ContaOsComandos;
    procedure Queue_CodificaUnifiedRequest;
    procedure Queue_SemArgumentos;
    procedure QueueArgs_MesmoResultado;
    procedure Clear_ZeraOLote;
    procedure LoteGrande_MantemAOrdem;
  end;

  TRedisHandshakeTests = class(TTestCase)
  published
    procedure Resp2_SemSenha_NaoEmiteNada;
    procedure Resp2_ComSenha_EmiteAuth;
    procedure Resp2_ComUsuario_EmiteAuthComUsuario;
    procedure ClientName_EmiteClientSetname;
    procedure Database_EmiteSelect;
    procedure DatabaseZero_NaoEmiteSelect;
    procedure Resp3_EmiteHello3;
    procedure Resp3_ComSenha_EmiteAuthComUsuarioDefault;
    procedure Resp3_LeVersaoEIdDoServidor;
    procedure Resp3_ServidorSemHello_ExplicaOMotivo;
    procedure SenhaErrada_LevantaEDeixaFechada;
    procedure OrdemDosComandos_AuthDepoisNomeDepoisSelect;
    procedure ConexaoNaoAberta_ExecuteLevanta;
  end;

  TRedisExecuteTests = class(TTestCase)
  published
    procedure Execute_EscreveUnifiedRequest;
    procedure Execute_DevolveARespostaLida;
    procedure Execute_ErroDoServidor_Levanta;
    procedure Execute_ErroDoServidor_NaoInvalidaAConexao;
    procedure ExecuteRaw_ErroDoServidor_NaoLevanta;
    procedure ExecuteArgs_ComandoDeDuasPalavras;
    procedure Execute_ArgumentoBinario_VaiIntacto;
    procedure Execute_RespostaBinaria_VoltaIntacta;
    procedure Execute_LeituraFatiada_MontaARespostaInteira;
    procedure Execute_EnvioFatiado_MandaOComandoInteiro;
    procedure Execute_FimDeFluxo_LevantaEInvalida;
    procedure Execute_DepoisDeInvalidada_RecusaSemIr;
    procedure Execute_RespostaSobrando_MarcaConexaoSuja;
    procedure Ping_PongDevolveTrue;
    procedure Ping_ErroDevolveFalse;
    procedure Select_AtualizaOBanco;
    procedure Select_ComErro_NaoAtualizaOBanco;
    procedure Close_FechaEPermiteDestruir;
    procedure Adotada_NaoReabreDepoisDoClose;
  end;

  TRedisConnectionPipelineTests = class(TTestCase)
  published
    procedure Pipeline_EnviaOLoteNumaEscritaSo;
    procedure Pipeline_UmaRespostaPorComando;
    procedure Pipeline_ErroNoMeio_NaoLevanta;
    procedure Pipeline_Vazio_NaoVaiAoServidor;
    procedure Pipeline_RespostaFaltando_LevantaEInvalida;
    procedure Pipeline_NaoDeixaResto_ConexaoContinuaLimpa;
  end;

  TRedisTlsTests = class(TTestCase)
  published
    procedure DefaultTlsParams_TrocaAPortaELigaOTls;
    procedure DefaultTlsParams_MantemAVerificacaoLigada;
    procedure BackendTlsDesteBuild_SeIdentifica;
    procedure ConexaoAdotada_IgnoraUseTls;
  end;
...
initialization
  RegisterTest(TRedisPipelineTests);
  RegisterTest(TRedisHandshakeTests);
  RegisterTest(TRedisExecuteTests);
  RegisterTest(TRedisConnectionPipelineTests);
  RegisterTest(TRedisTlsTests);
