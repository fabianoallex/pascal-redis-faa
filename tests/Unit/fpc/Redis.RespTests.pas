...
{ Testes do codec RESP (FPCUnit). Mesma cobertura do tests\Unit\Redis.RespTests.pas
  (DUnitX/Delphi) — as duas suites sao mantidas linha a linha equivalentes.
...
uses
  fpcunit, testregistry, SysUtils, Math, Redis.Types, Redis.Resp;

type
  TRedisCommandEncodeTests = class(TTestCase)
  published
    procedure Comando_SemArgumentos;
    procedure Comando_ComNomeEArgumentos;
    procedure Comando_FormaComNomeDestacado;
    procedure Comando_ArgumentoVazio;
    procedure Comando_ArgumentoInteiro;
    procedure Comando_ArgumentoComAcentos;
    procedure Comando_ArgumentoBinarioComCrLf;
    procedure Comando_ArgumentoGrande;
    procedure Comando_ListaVazia_Levanta;
  end;

  TRedisReadResp2Tests = class(TTestCase)
  published
    procedure SimpleString;
    procedure SimpleString_Vazia;
    procedure Erro_TemCodigoEMensagem;
    procedure Erro_SemEspaco_CodigoEhTudo;
    procedure Erro_RaiseIfError_Levanta;
    procedure NaoErro_RaiseIfError_NaoLevanta;
    procedure Inteiro;
    procedure Inteiro_Negativo;
    procedure Inteiro_Limite;
    procedure BulkString;
    procedure BulkString_Vazia_NaoEhNula;
    procedure BulkString_Nula;
    procedure BulkString_ComAcentos;
    procedure BulkString_BinarioComCrLf;
    procedure BulkString_Grande;
    procedure ArrayDeBulks;
    procedure ArrayVazio_NaoEhNulo;
    procedure ArrayNulo;
    procedure ArrayComNuloNoMeio;
    procedure ArrayAninhado;
    procedure ArrayAninhadoFundo;
    procedure ArrayComErroNoMeio;
  end;

  TRedisReadResp3Tests = class(TTestCase)
  published
    procedure Nulo;
    procedure Double_Simples;
    procedure Double_Infinito;
    procedure Double_Inteiro_TambemEhInteger;
    procedure Booleano_Verdadeiro;
    procedure Booleano_Falso;
    procedure BigNumber;
    procedure BlobError;
    procedure BlobError_ComCrLfNaMensagem;
    procedure Verbatim;
    procedure Verbatim_Markdown;
    procedure Mapa_ContaAchatado;
    procedure Mapa_ValueByKey;
    procedure Mapa_Aninhado;
    procedure Conjunto;
    procedure Push;
    procedure Atributo_NaoEhResposta;
    procedure Atributo_EmElementoDeArray;
    procedure ValueByKey_TambemEmArrayResp2;
    procedure ValueByKey_ChaveAusente_DevolveNil;
  end;

  TRedisPartialReadTests = class(TTestCase)
  private
    procedure ChecaEmTodosOsChunks(const AResp, ADescricao: string);
  published
    procedure SimpleString_ByteAByte;
    procedure BulkString_EmTodosOsChunks;
    procedure ArrayAninhado_EmTodosOsChunks;
    procedure RespostaResp3Completa_EmTodosOsChunks;
    procedure BulkGrande_EmChunkPequeno;
    procedure CrLfPartidoEntreLeituras;
  end;

  TRedisStreamStateTests = class(TTestCase)
  published
    procedure Buffered_ZeroAposRespostaCompleta;
    procedure Buffered_DetectaRespostaOrfa;
    procedure RespostasEmSequencia_MesmoLeitor;
    procedure FonteConsumidaPorInteiro;
  end;

  TRedisProtocolErrorTests = class(TTestCase)
  published
    procedure PrefixoDesconhecido;
    procedure LinhaVazia;
    procedure ComprimentoNaoNumerico;
    procedure ComprimentoNegativoInvalido;
    procedure ComprimentoAcimaDoTeto;
    procedure Streaming_NaoSuportado;
    procedure FaltouCrLfDepoisDaCarga;
    procedure FluxoCortadoNoMeio;
    procedure FluxoCortadoAntesDoPrimeiroByte;
    procedure BooleanoInvalido;
    procedure NuloComLixoDepois;
    procedure VerbatimSemPrefixoDeFormato;
    procedure BigNumberVazio;
    procedure AninhamentoAcimaDoTeto;
    procedure InteiroInvalido;
  end;

  TRedisTypeAccessTests = class(TTestCase)
  published
    procedure AsInteger_EmTextoNumerico;
    procedure AsInteger_EmTextoNaoNumerico_Levanta;
    procedure AsInteger_EmNulo_Levanta;
    procedure AsInteger_EmArray_Levanta;
    procedure AsInteger_EmDoubleFracionario_Levanta;
    procedure AsDouble_EmBulkString;
    procedure AsDouble_EmInteiro;
    procedure AsBoolean_NuloEhFalso;
    procedure AsBoolean_OkEhVerdadeiro;
    procedure AsBoolean_InteiroZeroEhFalso;
    procedure AsString_EmArray_Levanta;
    procedure AsBytes_EmNulo_EhVazio;
    procedure AsString_EmBulkBinario_NaoLevanta;
    procedure Items_EmEscalar_Levanta;
    procedure Items_ForaDosLimites_Levanta;
    procedure ValueByKey_EmEscalar_Levanta;
  end;

  TRedisReplyRoundTripTests = class(TTestCase)
  published
    procedure Resp3_ArvoreCompleta;
    procedure Resp2_RebaixaTiposDoResp3;
    procedure Resp2_MapaViraArrayAchatado;
    procedure BinarioSobrevive;
    procedure MapaImpar_Levanta;
  end;
...
initialization
  RegisterTest(TRedisCommandEncodeTests);
  RegisterTest(TRedisReadResp2Tests);
  RegisterTest(TRedisReadResp3Tests);
  RegisterTest(TRedisPartialReadTests);
  RegisterTest(TRedisStreamStateTests);
  RegisterTest(TRedisProtocolErrorTests);
  RegisterTest(TRedisTypeAccessTests);
  RegisterTest(TRedisReplyRoundTripTests);
