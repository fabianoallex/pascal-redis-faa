...
{ Testes de Redis.Transaction e Redis.Commands.Scripting (FPCUnit). Mesma
  cobertura do tests\Unit\Redis.TransactionTests.pas (DUnitX/Delphi) — as duas
...
uses
  ...
  Redis.Types, Redis.Resp, Redis.Connection, Redis.Commands,
  Redis.Commands.Scripting, Redis.Transaction, Redis.Client,
  Redis.ConnectionTests;

type
  TRedisScriptShaTests = class(TTestCase)
  published
    procedure Sha_TextoVazio;
    procedure Sha_VetorDeReferencia;
    procedure Sha_ScriptLua;
    procedure Sha_SaiEmMinusculas;
    procedure Sha_MedeOsBytesUtf8_NaoAString;
  end;

  TRedisTransactionTests = class(TTestCase)
  published
    procedure Commit_EnviaMultiComandosEExec;
    procedure Commit_DevolveUmaRespostaPorComando;
    procedure Commit_BlocoVazio_AindaVaiAoServidor;
    procedure Commit_ErroDeExecucao_VemComoItemESemRollback;
    procedure TryCommit_ExecNulo_DevolveFalse;
    procedure Commit_ExecNulo_LevantaAborted;
    procedure Commit_ComandoRecusadoNaFila_LevantaCitandoAPosicao;
    procedure Commit_MultiRecusado_Levanta;
    procedure Queue_DepoisDoCommit_Levanta;
    procedure Count_NaoConta0Multi;
    procedure Watch_VaiAoServidorNaHora;
    procedure Watch_SemChave_Levanta;
    procedure Commit_LimpaOEstadoDeWatch;
    procedure Discard_MandaUnwatchENaoMandaDiscard;
    procedure Discard_SemWatch_NaoVaiAoServidor;
    procedure Destrutor_ComWatchPendente_MandaUnwatch;
    procedure Destrutor_SemWatch_NaoMandaNada;
    procedure Destrutor_DevolveAConexaoAQuemEmprestou;
  end;

  TRedisScriptingTests = class(TTestCase)
  published
    procedure Eval_MontaNumkeysChavesEArgumentos;
    procedure Eval_SemChave_MandaZero;
    procedure Run_PrimeiraVez_MandaEval;
    procedure Run_SegundaVez_MandaEvalSha;
    procedure Run_Noscript_ReenviaOEval;
    procedure Run_ErroDeLua_LevantaEmVezDeReenviar;
    procedure Run_ScriptComErro_NaoEntraNoCache;
    procedure ScriptLoad_DevolveEMemorizaOSha;
    procedure ScriptFlush_LimpaOCacheLocal;
    procedure ScriptExists_LeOArrayDeUmElemento;
    procedure ClearCache_EsqueceSemFalarComOServidor;
  end;
...
initialization
  RegisterTest(TRedisScriptShaTests);
  RegisterTest(TRedisTransactionTests);
  RegisterTest(TRedisScriptingTests);
